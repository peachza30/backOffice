...
const DocumentViewerDialog: React.FC<DocumentViewerDialogProps> = ({ open, onClose, initialIndex }) => {
  const { request, documents, loading, fetchCorporateDocuments } = useCorporateStore();
  const [index, setIndex] = useState(initialIndex);
  const [src, setSrc] = useState("");
  const [files, setFiles] = useState(request?.document || []);
  ...
  useEffect(() => {
    if (index !== null && request && request.document && request.document.length > 0) {
      fetchCorporateDocuments(request.document[index].fileId);
      setFiles(request.document);
    }
  }, [index, request, fetchCorporateDocuments]);
  ...
  const goNext = () => setIndex(i => (i === null ? 0 : Math.min((request?.document?.length ?? 0) - 1, i + 1))); //Math.min(files.length - 1, i + 1))); //Math.min(request?.document.length - 1, i + 1)));
  ...
  const handlePrint = async () => {
    ...
    const currentFile = `/api/biz/download?fileId=${request?.document[currentIndex].fileId}&contentType=application/pdf&t=${Date.now()}`;
  ...
  const currentFile = request?.document[currentIndex];
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-7xl h-[85vh] flex flex-col gap-3 pl-14 pr-pl-14" size="full">
        ...
        <div className="relative flex-1 bg-gray-100 rounded border overflow-hidden group">
          ...
          {/* Right click‑strip */}
          <button aria-label="ถัดไป" onClick={goNext} disabled={request?.document && index === request.document.length - 1} className={`absolute inset-y-0 right-0 ${NAV_BTN_WIDTH} flex items-center justify-center transition-opacity bg-blue-50 hover:bg-blue-200/50  disabled:opacity-0 cursor-pointer`}>
        ...
        <div className="">
          ...
          <RadioGroup defaultValue="right" className="my-7 ">
            ...
            <RadioGroupItem value="3" size="xs" id="r_3" color="default">
              ยังไม่ตรวจ
            </RadioGroupItem>
            <RadioGroupItem value="4" size="xs" id="r_4" color="default">
              ไม่ใช้งาน
            </RadioGroupItem>
          </RadioGroup>
          <label>หมายเหตุ</label>
          <Textarea className="my-3 " placeholder="กรุณากรอกหมายเหตุ" rows={3} />
        </div>
