...
import { Table, TableBody, TableCell, TableRow } from "./staff-audit-custom-table";
...
const StaffAuditList: React.FC<{ items: StaffSummaryItem[] }> = ({ items }) => (
        ...
        <TableRow key={`${label}-${idx}`} className="even:bg-blue-50/50">
          <TableCell>
            <div className="flex items-center whitespace-nowrap font-bold">
              <Icon icon={icon} width="20" height="20" className="mr-2 text-blue-600" />
              <span>{label}</span>
            </div>
          </TableCell>
          <TableCell className="whitespace-nowrap text-left">{count}</TableCell>
