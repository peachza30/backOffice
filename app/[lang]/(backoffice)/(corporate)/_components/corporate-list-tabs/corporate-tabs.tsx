...
export default function CorporateView({ id }: { id: number }) {
  ...
  useEffect(() => {
    setGuaranteeType(corporate?.guarantee[0].guaranteeTypeId || "");
  }, [corporate?.guarantee]);
  console.log("corporate", corporate);

  /*************  ✨ Windsurf Command ⭐  *************/
  /**
   * Navigate to the corporate edit page if the id is provided.
   * @example
   * <button onClick={handleEdit}>Edit</button>
   */
  /*******  a3755338-95c4-4bed-befa-7d4cdc4ba68a  *******/ const handleEdit = () => {
    if (id) {
            ...
            <TabsContent value="page1" className="border-collapse">
              <Card className="mb-4 border-2 border-blue-100/75">
                ...
                <CardContent className="space-y-2 ">
                  <div className="grid grid-cols-1 gap-6">
                    {[
                      { label: "ประเภทนิติบุคคล", value: corporate.corporateServiceName || "-" },
                      { label: "เลขประจำตัวผู้เสียภาษี", value: corporate.registrationNo || "-" },
              ...
              <Card className="mb-4 border-2 border-blue-100/75">
                ...
                <CardContent className="space-y-2">
                  <div className="grid grid-cols-1 gap-6">
                    {[
                      { label: "วันเริ่มประกอบธุรกิจ", value: corporate.expiredDate ? toBuddhistDate(corporate.nameTh) : "-" },
                      { label: "วันที่ยื่นจดทะเบียนต่อสภา", value: corporate.expiredDate ? toBuddhistDate(corporate.nameEn) : "-" },
                      { label: "วันที่เริ่มต้นสถานภาพ", value: corporate.expiredDate ? toBuddhistDate(corporate.beginDate) : "-" },
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <p className="text-sm text-gray-900 p-2 rounded flex-1">{corporate.nameTh || "-"}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              <Card className="mb-4 border-2 border-blue-100/75">
                ...
                <CardContent className="space-y-2">
                  <div className="grid grid-cols-1 gap-6">
                    {[{ label: "เงินทุนจดทะเบียน", value: (corporate.capital && formatCurrency(corporate.capital)) || "บริษัท เอ็ม แอนด์ เอ็ม แอ็คเค้าท์ติ้ง จำกัด" }].map(({ label, value }) => (
                      <div className="flex items-start gap-4" key={label}>
                        <label className="block text-sm font-bold text-gray-700 pt-2" style={{ width: "140px", minWidth: "140px" }}>
                          {label}
                        </label>
                  ...
                  <CardTitle className="text-md font-bold">
                    <div className="flex flex-wrap justify-evenly gap-y-4">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Input type="text" value={(corporate.accountingRevenue && formatCurrency(corporate.accountingRevenue)) || "0.00"} inputMode="numeric" className="w-28 text-center" placeholder="0.00" />
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Input type="text" value={(corporate.auditingRevenue && formatCurrency(corporate.auditingRevenue)) || "0.00"} inputMode="numeric" className="w-28 text-center" placeholder="0.00" />
                        <span className="font-bold">บาท</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Input type="text" value={(corporate.otherRevenue && formatCurrency(corporate.otherRevenue)) || "0.00"} inputMode="numeric" className="w-28 text-center" placeholder="0.00" />
                        <span className="font-bold">บาท</span>
                      </div>
            ...
            <TabsContent value="page4" className="border-collapse">
              <Card className="mb-4 border-2 border-blue-100/75">
                ...
                <CardContent className="space-y-2">
                  ...
                  <GuaranteesDetails rows={corporate} />
                  <div className="p-4">
                    <p>
                  ...
                  <div className="pl-5 pt-3 pb-5 grid grid-cols-1 gap-6">
                    {[
                      ...
                      { label: "คิดเป็นจำนวนเงิน : ", value: corporate.nameEn || "-" },
                    ].map(({ label, value }) => (
                      <div className="flex items-start gap-4" key={label}>
                        <label className="block text-sm font-bold text-gray-700" style={{ width: "140px", minWidth: "140px" }}>
                          {label}
              ...
              <Card className="mb-4 border-2 border-blue-100/75">
                <CardHeader className="bg-blue-50/50">
                  <CardTitle className="font-bold">รายละเอียดหลักประกัน : {corporate.guarantee[0].guaranteeTypeName || "-"}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
