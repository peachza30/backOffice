...
const MenuManagement: React.FC = () => {
  ...
  const handleDelete = (item: MenuItem): void => {
    ...
    if (confirm(`Are you sure you want to delete "${item.menu_name}"?`)) {
      ...
      const updatedMenus = deleteMenuItem(menusList);
      setMenusList(resequenceItems(updatedMenus));
    }
  };
  ...
  const saveChanges = async () => {
    ...
    // Create a summary of changes for user confirmation
    const updates = changes.filter(c => !c.is_new && !c.is_deleted);
    const creates = changes.filter(c => c.is_new);
    const deletes = changes.filter(c => c.is_deleted);
    ...
    // alert(`Saving ${changes.length} changes...`);

    // After successful save, update original menus
    // await updateMenu(menu.id, menuData);
    setOriginalMenus(JSON.parse(JSON.stringify(menusList)));
      ...
      <div className="">
        <div className="">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                {/* <p className="text-sm text-gray-500 mt-1">
                  {isReorderMode ? (
                    <>
                      Drag and drop to reorder menus. Drop on items to nest them.
                      <span className="inline-block ml-2 text-amber-600 font-medium">
                        <Icon icon="mdi:circle" className="w-2 h-2 inline mr-1" />
                        Modified items are highlighted
                      </span>
                    </>
                  ) : (
                    "Click on parent items to expand/collapse. Use action buttons to manage menus."
                  )}
                </p> */}
              </div>
          ...
          <div className="p-6 border-t bg-gray-50 flex justify-between">
            ...
            >
              {isReorderMode ? "Cancel" : "Back"}
            </Button>

            {isReorderMode && (
              <Button onClick={saveChanges} disabled={compareMenus(originalMenus, menusList).length === 0}>
                Save Changes ({compareMenus(originalMenus, menusList).length})
              </Button>
            )}
          </div>
        </div>
