...
/**
 * Map API menus to permission items format (recursive)
 */
    ...
    (set, get) => ({
      ...
      fetchRolesScope: async (scope) => {
        ...
        try {
          ...
          set({ roleScope: res.data, metadata: res.metadata, loading: false });
        } catch (err) {
          if (err instanceof Error) {
            set({ error: err.message, loading: false });
          } else {
      ...
      togglePermission: (category, itemId, permissionType) => {
        ...
        set((state) => {
          ...
          return {
            permissionItems: {
              ...state.permissionItems,
              [category]: updatePermission(state.permissionItems[category])
            }
          };
        });
      },
      toggleAllPermissions: (category, itemId) => {
        ...
        set((state) => {
          ...
          return {
            permissionItems: {
              ...state.permissionItems,
              [category]: updateAllPermissions(state.permissionItems[category])
            }
          };
        });
      },

      // Troubleshooting: If level 2 still can't expand to show level 3
      // 1. Check that your menu data has correct parent_id relationships
      // 2. Use this helper to verify the hierarchy is built correctly:
      ...
      submitRole: async () => {
          ...
          currentItems.forEach(item => {
            ...
            if (!original) {
              ...
              if (changedFields.length > 0) {
                ...
                console.log("Original:", original.permissions);
                console.log("Current :", item.permissions);
                collector(item, isService);
              }
            }
        ...
        //     await get().updateRole(state.role.role_id, payload);
        //   } else {
...
const applyRolePermissionsToItems = (
  ...
  rolePermissions: ServicePermission[] | PermissionItem[], // Changed from MenuItem[] to PermissionItem[]
  type: 'service' | 'menu'
): PermissionItem[] => {
  ...
  rolePermissions.forEach(perm => {
    const key = type === 'service'
      ? (perm as ServicePermission).service_id
      : (perm as PermissionItem).id; // Changed to PermissionItem
